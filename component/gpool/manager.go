...
func NewGPoolManager() core.IGPoolManager {
	return &gpoolManager{
		conn: conn.NewConn(),
	}
}

func (g *gpoolManager) GetGPool(name ...string) core.IGPool {
	return g.conn.GetInstance(g.makeGPool, name...).(core.IGPool)
}

func (g *gpoolManager) makeGPool(name string) (conn.IInstance, error) {
